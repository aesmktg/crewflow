...
const DEFAULT_MASTER_ITEMS = [
  { id:"mi1",  name:"Truss Display",              category:"Staging"  },
  { id:"mi2",  name:"Baby Color Indoor LED Lights",category:"Lighting" },
  { id:"mi3",  name:"Sand Bags",                   category:"Rigging"  },
  { id:"mi4",  name:"Genie Lift",                  category:"Staging"  },
  { id:"mi5",  name:"QSC K12.2 Speaker",           category:"Audio"    },
  { id:"mi6",  name:"QSC K-Sub Subwoofer",         category:"Audio"    },
  { id:"mi7",  name:"Shure SM58 Microphone",       category:"Audio"    },
  { id:"mi8",  name:"XLR Cable 25ft",              category:"Audio"    },
  { id:"mi9",  name:"Pipe & Base",                 category:"Staging"  },
  { id:"mi10", name:"Drape Panel",                 category:"Staging"  },
  { id:"mi11", name:"LED Par Can",                 category:"Lighting" },
  { id:"mi12", name:"DMX Cable",                   category:"Lighting" },
  { id:"mi13", name:"Moving Head Light",           category:"Lighting" },
  { id:"mi14", name:"Extension Cord 50ft",         category:"Power"    },
  { id:"mi15", name:"Power Strip",                 category:"Power"    },
  { id:"mi16", name:"Generator",                   category:"Power"    },
  { id:"mi17", name:"Projector",                   category:"Video"    },
  { id:"mi18", name:"Projection Screen",           category:"Video"    },
  { id:"mi19", name:"HDMI Cable",                  category:"Video"    },
  { id:"mi20", name:"LED Video Panel",             category:"Video"    },
  { id:"mi21", name:"Chain Motor",                 category:"Rigging"  },
  { id:"mi22", name:"Shackles",                    category:"Rigging"  },
  { id:"mi23", name:"Safety Cable",                category:"Rigging"  },
  { id:"mi24", name:"Drum Kit",                    category:"Backline" },
  { id:"mi25", name:"Guitar Amplifier",            category:"Backline" },
  { id:"mi26", name:"Keyboard Stand",              category:"Backline" },
  { id:"mi27", name:"Table Linen",                 category:"Décor"    },
  { id:"mi28", name:"Centerpiece",                 category:"Décor"    },
  { id:"mi29", name:"Gaffer Tape",                 category:"Misc"     },
  { id:"mi30", name:"Work Gloves",                 category:"Misc"     },
];

const DEFAULT_FLEET = {
  trucks: [
    { id:"tr1", name:"Box Truck #1",   detail:"Ford F-650"          },
    { id:"tr2", name:"Box Truck #2",   detail:"GMC TopKick"         },
    { id:"tr3", name:"Sprinter Van #1",detail:"Mercedes Sprinter"   },
    { id:"tr4", name:"Sprinter Van #2",detail:"Mercedes Sprinter"   },
    { id:"tr5", name:"Pickup #1",      detail:"Ford F-250"          },
    { id:"tr6", name:"Pickup #2",      detail:"Chevy Silverado 2500"},
  ],
  trailers: [
    { id:"tl1", name:"24ft Enclosed #1",detail:"Pace American" },
    { id:"tl2", name:"24ft Enclosed #2",detail:"Cargo Mate"    },
    { id:"tl3", name:"16ft Open Flatbed",detail:"Big Tex"      },
    { id:"tl4", name:"20ft Enclosed",   detail:"Haulmark"      },
    { id:"tl5", name:"Utility Trailer", detail:"PJ Trailers"   },
  ],
};

const DEFAULT_USERS = [
  { id:"admin",  name:"Admin",     pin:"0000", role:"admin",    email:"admin@company.com",   active:true, createdAt:new Date().toISOString() },
  { id:"emp1",   name:"Kevin W.",  pin:"1234", role:"employee", email:"kevin@company.com",   active:true, createdAt:new Date().toISOString() },
  { id:"emp2",   name:"Charlie O.",pin:"1234", role:"employee", email:"charlie@company.com", active:true, createdAt:new Date().toISOString() },
  { id:"emp3",   name:"Stephen B.",pin:"1234", role:"employee", email:"stephen@company.com", active:true, createdAt:new Date().toISOString() },
  { id:"emp4",   name:"Darren M.", pin:"1234", role:"employee", email:"darren@company.com",  active:true, createdAt:new Date().toISOString() },
];
...
:root{
  ...
  --fa:#1E1E2A;
  --ac:#E8FF47;
  --ac2:rgba(232,255,71,0.13);
  --dn:#FF4040;
  --dn2:rgba(255,64,64,0.12);
  --wn:#F59E0B;
  --wn2:rgba(245,158,11,0.12);
  --ok:#10B981;
  --ok2:rgba(16,185,129,0.12);
  --bl:#3B82F6;
  --bl2:rgba(59,130,246,0.12);
  --pu:#8B5CF6;
  --pu2:rgba(139,92,246,0.12);
...
.acitem:hover,.acitem:active{background:var(--s2)}
...
function AutocompleteInput({ value, onChange, masterItems, placeholder }) {
  ...
  const filtered = value.length > 0
    ? masterItems.filter(m => m.name.toLowerCase().includes(value.toLowerCase())).slice(0, 8)
    : [];
  ...
  return (
    <div className="acwrap" ref={ref}>
      <input className="fi" value={value}
    ...
    <div className="mback ctr">
      ...
      <div className="modal">
        ...
        <div className="field">
          ...
          {truckVal === "__custom" && (
            <input className="fi" style={{ marginTop:8 }} value={customTruck} onChange={e => setCustomTruck(e.target.value)} placeholder="Enter truck name" />
          )}
        </div>
        <div className="field">
          ...
          {trailerVal === "__custom" && (
            <input className="fi" style={{ marginTop:8 }} value={customTrailer} onChange={e => setCustomTrailer(e.target.value)} placeholder="Enter trailer name" />
          )}
        </div>
        <div className="macts">
...
function ItemModal({ item, onSave, onClose, masterItems }) {
  ...
  const handleSave = () => {
    ...
    const base = isEdit
      ? { ...item }
      : { id:uid(), status:"pending", addedBy:null, statusLog:[], removedAt:null };
    ...
    <div className="mback">
      ...
      <div className="modal">
        ...
        <div className="field">
          ...
          <AutocompleteInput value={name} onChange={setName} masterItems={masterItems} />
        </div>
        <div className="frow">
          ...
          <div className="field"><label className="flbl">Unit</label>
        ...
        <div className="field"><label className="flbl">Notes (optional)</label>
          <textarea className="fta" value={notes} onChange={e => setNotes(e.target.value)} placeholder="Special instructions…" rows={2} />
        </div>
        <div className="macts">
          <button className="btn bghost" onClick={onClose}>Cancel</button>
...
function EventForm({ onSave, onClose, existing, masterItems }) {
  ...
  const addItem = () => {
    ...
    setItems(prev => [...prev, {
      id:uid(), name:ni.name.trim(), qty:`${ni.qty} ${ni.unit}`,
      category: match ? match.category : ni.cat,
      status:"pending", addedBy:"admin", statusLog:[], removedAt:null, notes:""
    }]);
  ...
  const save = (live) => {
    if (!name.trim()) return;
    onSave({
      ...e, id:e.id || uid(), name, venue, address,
      eventStart, eventEnd, installDT, strikeDT, departureDT, brief,
      items:[...items, ...(e.items || []).filter(i => i.removedAt)],
      live, archived:e.archived || false,
      createdAt:e.createdAt || now(), updatedAt:now(),
      auditLog:e.auditLog || [], activityLog:e.activityLog || [],
      truck:e.truck || "", trailer:e.trailer || ""
    });
  };
  ...
  return (
    <div className="mback">
      ...
      <div className="modal">
        ...
        <div className="field"><label className="flbl">Venue Name</label><input className="fi" value={venue} onChange={e => setVenue(e.target.value)} placeholder="e.g. The Grand Ballroom" /></div>
        <div className="field"><label className="flbl">Address</label><input className="fi" value={address} onChange={e => setAddress(e.target.value)} placeholder="123 Main St, City, State" /></div>
        <div className="frow">
        ...
        <div className="field"><label className="flbl">Project Brief / Admin Notes</label>
          <textarea className="fta" value={brief} onChange={e => setBrief(e.target.value)} placeholder="Crew notes — admin-only editing." rows={3} />
        </div>
        <div className="field">
          ...
          <div style={{ background:"var(--s2)", border:"1px solid var(--br)", borderRadius:"var(--r)", padding:11 }}>
            ...
            <div style={{ display:"flex", gap:6, flexWrap:"wrap", marginTop:7 }}>
              <div style={{ flex:"2 1 120px" }}>
                <AutocompleteInput value={ni.name} onChange={v => setNi(p => ({ ...p, name:v }))} masterItems={masterItems} placeholder="Search or type item…" />
              </div>
              <input className="fi" style={{ width:52 }} type="number" value={ni.qty} onChange={e => setNi(p => ({ ...p, qty:e.target.value }))} min="1" />
              <select className="fsel" style={{ flex:"1 1 65px" }} value={ni.unit} onChange={e => setNi(p => ({ ...p, unit:e.target.value }))}>
                {UNITS.map(u => <option key={u}>{u}</option>)}
              </select>
              <select className="fsel" style={{ flex:"1 1 90px" }} value={ni.cat} onChange={e => setNi(p => ({ ...p, cat:e.target.value }))}>
                {categories.map(c => <option key={c}>{c}</option>)}
              </select>
...
function EventDetail({ event, user, onBack, onUpdate, masterItems, fleet }) {
  ...
  const mkLog = (action, detail) => ({ id:uid(), action, detail, by:user.name, userId:user.id, at:now(), eventId:event.id });
  const pt = (msg, type) => setToast({ msg, type });

  // Auto-archive when all returned
  const statusKey = active.map(i => i.status).join(",");
  useEffect(() => {
    if (active.length > 0 && active.every(i => i.status === "returned") && !event.archived) {
      onUpdate({ ...event, archived:true, archivedAt:now(), archivedReason:"auto" });
      pt("All items returned — event auto-archived ✓", "ok");
    }
  }, [statusKey]);

  const handleStatus = (status) => {
    const item = statusTarget;
    const l = mkLog("status_change", `"${item.name}": ${item.status} → ${status}`);
    const updated = { ...item, status, statusLog:[...(item.statusLog||[]), { status, by:user.name, at:now() }], [`${status}By`]:user.name, [`${status}At`]:now() };
    onUpdate({ ...event, items:items.map(i => i.id === item.id ? updated : i), activityLog:[...(event.activityLog||[]), l] });
    setStatusTarget(null);
    pt(`${item.name} → ${STATUS_CONFIG[status].label}`, "ok");
  };

  const handleAddItem = (item) => {
    const l = mkLog("item_added", `Added "${item.name}" (${item.qty}) [${item.category}]`);
    const ae = { type:"add", itemName:item.name, qty:item.qty, by:user.name, at:now() };
    onUpdate({ ...event, items:[...items, { ...item, addedBy:user.name, addedByUserId:user.id, addedAt:now() }], auditLog:[...(event.auditLog||[]), ae], activityLog:[...(event.activityLog||[]), l] });
    setShowAdd(false);
  ...
  const handleEditItem = (updated) => {
    ...
    const l = mkLog("item_modified", `Modified "${orig.name}": ${changes.join(", ")}`);
    const ae = { type:"mod", itemName:orig.name, changes:changes.join(", "), by:user.name, at:now() };
    onUpdate({ ...event, items:items.map(i => i.id === updated.id ? { ...updated, modifiedBy:user.name, modifiedAt:now() } : i), auditLog:[...(event.auditLog||[]), ae], activityLog:[...(event.activityLog||[]), l] });
    setEditTarget(null);
    pt("Item updated", "ok");
  };
  ...
  const handleRemove = () => {
    ...
    const l = mkLog("item_removed", `Removed "${item.name}" (${item.qty}) [${item.category}] — was ${item.status}`);
    const ae = { type:"rm", itemName:item.name, qty:item.qty, category:item.category, by:user.name, at:now(), prevStatus:item.status };
    onUpdate({ ...event, items:items.map(i => i.id === item.id ? { ...i, removedAt:now(), removedBy:user.name, removedByUserId:user.id } : i), auditLog:[...(event.auditLog||[]), ae], activityLog:[...(event.activityLog||[]), l] });
    setRemoveTarget(null);
    pt(`"${item.name}" removed`, "err");
  };

  const handleTruck = (truck, trailer) => {
    const l = mkLog("vehicle_update", `Vehicle: "${truck}" / Trailer: "${trailer}"`);
    onUpdate({ ...event, truck, trailer, activityLog:[...(event.activityLog||[]), l] });
    setShowTruck(false);
    ...
    <div>
      ...
      <div className="dh">
        <div className="dh-top">
          ...
          <div className="pills">
            <span className={"pill " + (event.archived ? "parc" : event.live ? "plive" : "pdraft")}>
              {event.archived ? "Archived" : event.live ? "Live" : "Draft"}
            </span>
            {isAdmin && !event.archived && <button className="btn bacc bsm" onClick={() => setShowEditEvent(true)}>Edit</button>}
            {isAdmin && !event.archived && (
              <button className="btn bghost bsm" onClick={() => { onUpdate({ ...event, archived:true, archivedAt:now(), archivedReason:"manual" }); onBack(); }}>Archive</button>
            )}
          </div>
        </div>
        <div className="dgrid">
        ...
        <div className="trchip" onClick={() => !event.archived && setShowTruck(true)}>
          ...
          <div style={{ flex:1 }}>
            ...
            <div className="icv">
              {event.truck || event.trailer
                ? `${event.truck || "—"} · ${event.trailer || "—"}`
                : <span style={{ color:"var(--mu)", fontSize:12 }}>Tap to assign vehicle</span>}
            </div>
          </div>
      ...
      <div className="pblock">
        ...
        <div className="spills">
          {Object.entries(STATUS_CONFIG).map(([k, v]) => (
            <span key={k} className="spill" style={{ color:v.color, borderColor:`${v.color}44`, background:v.bg }}>
              {active.filter(i => i.status === k).length} {v.label}
            </span>
          ))}
        </div>
      </div>
      ...
      {event.brief && (
        <div className="brief"><div className="brieft">📋 Admin Brief</div><div className="briefb">{event.brief}</div></div>
      )}
      ...
      {Object.keys(byCategory).length === 0 && (
        <div className="empty"><div className="eico">📦</div><div className="etxt">No items yet.</div></div>
      )}
            ...
            return (
              <div key={item.id} className="irow">
                <div className={"ichk " + (item.status || "pending")}
                  onClick={() => !event.archived && setStatusTarget(item)}
                  style={event.archived ? { opacity:.5, cursor:"default" } : { cursor:"pointer" }}>
                  {item.status !== "pending" && (
                    <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke={sc.color} strokeWidth="3"><polyline points="20 6 9 17 4 12" /></svg>
                  )}
                </div>
                <div className="imain">
      ...
      {(removed.length > 0 || (event.auditLog || []).length > 0) && (
        <div className="auditsec">
          ...
          {(event.auditLog || []).filter(l => l.type === "mod").map((l, i) => (
            <div key={i} className="aurow mod">
              <div className="auitem">✏ {l.itemName}</div>
              <div className="aumeta">{l.changes} · by {l.by} · {fmtDT(l.at)}</div>
            </div>
          ))}
          {(event.auditLog || []).filter(l => l.type === "add" && l.by !== "admin").map((l, i) => (
            <div key={i} className="aurow add">
              <div className="auitem">✚ {l.itemName} ({l.qty})</div>
              <div className="aumeta">Added by {l.by} · {fmtDT(l.at)}</div>
            </div>
          ))}
        </div>
      )}
      ...
      {removeTarget && (
        <div className="mback ctr">
          <div className="mover" onClick={() => setRemoveTarget(null)} />
          <div className="modal">
            <Confirm title="Remove Item?" body={`Remove "${removeTarget.name}" (${removeTarget.qty})? This will be logged.`} danger onConfirm={handleRemove} onCancel={() => setRemoveTarget(null)} confirmLabel="Yes, Remove" />
          </div>
        </div>
      )}
      ...
      {showEditEvent && <EventForm existing={event} masterItems={masterItems} onSave={ev => { onUpdate(ev); setShowEditEvent(false); pt("Event updated!", "ok"); }} onClose={() => setShowEditEvent(false)} />}
    </div>
  );
}
...
function EventList({ events, user, onSelect, onCreateNew }) {
  ...
  return (
    <div>
      ...
      {visible.length === 0 && (
        <div className="empty">
          <div className="eico">{tab === "active" ? "🎪" : "📁"}</div>
          <div className="etxt">{tab === "active" ? (isAdmin ? "No events. Create one!" : "No live events right now.") : "No archived events."}</div>
        </div>
      )}
      {visible.map(ev => {
        ...
        return (
          <div key={ev.id} className={"ecard" + (ev.archived ? " arc" : "")} onClick={() => onSelect(ev)}>
            <div className="ehd">
              ...
              <div className="pills">
                <span className={"pill " + (ev.archived ? "parc" : ev.live ? "plive" : "pdraft")}>{ev.archived ? "Archived" : ev.live ? "Live" : "Draft"}</span>
              </div>
            </div>
            <div className="emeta">
...
// ─── ACTIVITY LOG ─────────────────────────────────────────────────────────────
function ActivityLog({ events, users }) {
  const [filter, setFilter] = useState("all");
  const allLogs = events
    .flatMap(ev => (ev.activityLog || []).map(l => ({ ...l, eventName:ev.name })))
    .sort((a, b) => new Date(b.at) - new Date(a.at));
  const empIds = [...new Set(allLogs.map(l => l.userId))];
  const filtered = filter === "all" ? allLogs : allLogs.filter(l => l.userId === filter);
    ...
    <div>
      <div className="tabrow">
        ...
        {empIds.map(id => {
          ...
          return <button key={id} className={"btn bsm " + (filter === id ? "bacc" : "bghost")} onClick={() => setFilter(id)}>{u ? u.name : id}</button>;
        })}
      </div>
      ...
      <div style={{ background:"var(--sf)", border:"1px solid var(--br)", borderRadius:"var(--rl)", overflow:"hidden" }}>
        {filtered.slice(0, 150).map((l, i) => {
          const u = resolveUser(l.userId);
          return (
            <div key={i} className="logrow">
              <div className="logav">{(u ? u.name : l.by || "?")[0]}</div>
              <div className="logbody">
                <div className="logact"><strong style={{ color:"var(--tx)" }}>{l.by}</strong> — {l.detail}</div>
                <div className="logt">{fmtFull(l.at)}</div>
                <div className="logevt">Event: {l.eventName}</div>
              </div>
            </div>
          );
        })}
...
function UserManager({ users, onUpdate }) {
  ...
  const handleAdd = () => {
    ...
    onUpdate([...users, { id:uid(), name:form.name.trim(), pin:form.pin, email:form.email, role:form.role, active:true, createdAt:now() }]);
    setShowAdd(false);
    setForm({ name:"", pin:"", email:"", role:"employee" });
  ...
  const handleDeactivate = () => {
    onUpdate(users.map(u => u.id === confirmDel.id ? { ...u, active:false, deactivatedAt:now() } : u));
    setConfirmDel(null);
    pt(`${confirmDel.name} deactivated — data retained`, "ok");
  };
    ...
    <div>
      ...
      {users.filter(u => u.role !== "admin").map(u => (
        <div key={u.id} className="ucard">
          ...
          <div className="ucinfo">
            ...
            {!u.active && u.deactivatedAt && <div style={{ fontSize:10, color:"var(--dn)", marginTop:2 }}>Deactivated {fmtDT(u.deactivatedAt)} — history retained</div>}
          </div>
          <div className="ucacts">
            {u.active ? (
              ...
              <button className="btn bok bsm" onClick={() => onUpdate(users.map(x => x.id === u.id ? { ...x, active:true, deactivatedAt:null } : x))}>Restore</button>
            )}
          </div>
        </div>
      ))}
      {showAdd && (
        <div className="mback ctr"><div className="mover" onClick={() => setShowAdd(false)} />
          <div className="modal">
            ...
            <div className="field"><label className="flbl">Email</label><input className="fi" type="email" value={form.email} onChange={e => setForm(p => ({ ...p, email:e.target.value }))} /></div>
      ...
      {editUser && (
        <div className="mback ctr"><div className="mover" onClick={() => setEditUser(null)} />
          <div className="modal">
            ...
            <div className="field"><label className="flbl">Email</label><input className="fi" type="email" value={editUser.email} onChange={e => setEditUser(p => ({ ...p, email:e.target.value }))} /></div>
      ...
      {confirmDel && (
        <div className="mback ctr"><div className="mover" onClick={() => setConfirmDel(null)} />
          <div className="modal">
            <Confirm title="Deactivate User?" body={`${confirmDel.name} will be locked out. All their activity and entries will be fully preserved.`} danger onConfirm={handleDeactivate} onCancel={() => setConfirmDel(null)} confirmLabel="Deactivate" />
          </div>
        </div>
      )}
    </div>
  ...
  return (
    <div>
      ...
      {showAdd && (
        <div className="mback ctr"><div className="mover" onClick={() => setShowAdd(false)} />
          <div className="modal">
            ...
            <div className="field"><label className="flbl">Item Name *</label><input className="fi" value={newItem.name} onChange={e => setNewItem(p => ({ ...p, name:e.target.value }))} placeholder="e.g. Truss Display 10ft" autoFocus /></div>
            <div className="field"><label className="flbl">Category</label>
            ...
            <div className="macts">
              <button className="btn bghost" onClick={() => setShowAdd(false)}>Cancel</button>
              <button className="btn bprim" style={{ flex:2 }} onClick={() => {
                if (!newItem.name.trim()) return;
                onUpdate([...masterItems, { id:uid(), name:newItem.name.trim(), category:newItem.category }]);
      ...
      {confirmDel && (
        <div className="mback ctr"><div className="mover" onClick={() => setConfirmDel(null)} />
          <div className="modal">
            <Confirm title="Remove from Library?" body={`Remove "${confirmDel.name}"? This won't affect existing events.`} danger
              onConfirm={() => { onUpdate(masterItems.filter(m => m.id !== confirmDel.id)); setConfirmDel(null); setToast({ msg:"Removed", type:"err" }); }}
...
function CategoryManager({ categories, onUpdate }) {
  ...
  const handleAdd = () => {
    ...
    if (categories.map(c => c.toLowerCase()).includes(trimmed.toLowerCase())) {
      setToast({ msg:"Category already exists", type:"err" }); return;
    }
    onUpdate([...categories, trimmed]);
    setNewCat("");
    setToast({ msg:`"${trimmed}" added`, type:"ok" });
  };
  ...
  const handleEdit = (idx) => {
    ...
    onUpdate(updated);
    setEditIdx(null);
    setToast({ msg:"Category updated", type:"ok" });
  };

  const handleDelete = (idx) => {
    onUpdate(categories.filter((_, i) => i !== idx));
    setConfirmDel(null);
    setToast({ msg:"Category removed", type:"err" });
  };
  ...
  return (
    <div>
      ...
      <div className="infobanner">
        Categories are used to organize gear items across all events. Editing a category name here does not retroactively update items already assigned to it.
      </div>
      <div style={{ display:"flex", gap:8, marginBottom:14 }}>
        <input className="fi" value={newCat} onChange={e => setNewCat(e.target.value)}
      ...
      {categories.map((cat, idx) => (
        <div key={idx} className="mitem">
          {editIdx === idx ? (
            <>
              <input className="fi" value={editVal} onChange={e => setEditVal(e.target.value)}
      ...
      {confirmDel !== null && (
        <div className="mback ctr"><div className="mover" onClick={() => setConfirmDel(null)} />
          <div className="modal">
            <Confirm title="Remove Category?"
              body={`Remove "${categories[confirmDel]}"? Items already using this category won't be affected.`}
              danger onConfirm={() => handleDelete(confirmDel)} onCancel={() => setConfirmDel(null)} confirmLabel="Remove" />
          </div>
        </div>
      )}
    </div>
...
function FleetLibrary({ fleet, onUpdate }) {
  ...
  const handleAdd = () => {
    if (!newItem.name.trim()) return;
    const entry = { id:uid(), name:newItem.name.trim(), detail:newItem.detail.trim() };
    const updated = showAdd === "truck"
      ? { ...fleet, trucks:[...trucks, entry] }
      : { ...fleet, trailers:[...trailers, entry] };
    onUpdate(updated);
  ...
  const handleDel = () => {
    ...
    onUpdate(updated);
    setConfirmDel(null);
    setToast({ msg:"Removed from fleet", type:"err" });
  };
  ...
  return (
    <div>
      ...
      {showAdd && (
        <div className="mback ctr"><div className="mover" onClick={() => setShowAdd(null)} />
          <div className="modal">
            ...
            <div className="field"><label className="flbl">Make / Model / Detail</label>
      ...
      {confirmDel && (
        <div className="mback ctr"><div className="mover" onClick={() => setConfirmDel(null)} />
          <div className="modal">
            <Confirm title="Remove Vehicle?" body={`Remove "${confirmDel.name}" from the fleet list?`} danger onConfirm={handleDel} onCancel={() => setConfirmDel(null)} confirmLabel="Remove" />
          </div>
        </div>
      )}
    </div>
            ...
            {userId && (
              <>
                <div className="field">
                  ...
                  <div className="pdots">
                    {[0,1,2,3].map(i => <div key={i} className={"pdot" + (i < pin.length ? " on" : "")}>{i < pin.length ? "●" : ""}</div>)}
                  </div>
                </div>
                <div className="pgrid">
                  {["1","2","3","4","5","6","7","8","9","⌫","0","→"].map((k, i) => (
                    <button key={i} className={"pkey" + (k === "→" ? " go" : "")}
                      style={k === "→" ? { opacity:.35, cursor:"default" } : {}}
                      onClick={() => k === "⌫" ? handlePad("back") : k !== "→" ? handlePad(k) : null}>
                      {k}
                    </button>
                  ))}
                </div>
...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      const ev = await get("cf:events");
      const us = await get("cf:users");
      const mi = await get("cf:masterItems");
      const fl = await get("cf:fleet");
      const ca = await get("cf:categories");
      setEvents(ev || []);
      setUsers(us || DEFAULT_USERS);
      setMasterItems(mi || DEFAULT_MASTER_ITEMS);
      setFleet(fl || DEFAULT_FLEET);
      setCategories(ca || DEFAULT_CATEGORIES);
    })();
  }, []);

  const saveEvents = async (evs) => { setEvents(evs); await set("cf:events", evs); };
  const saveUsers = async (us) => { setUsers(us); await set("cf:users", us); };
  const saveMaster = async (mi) => { setMasterItems(mi); await set("cf:masterItems", mi); };
  const saveFleet = async (fl) => { setFleet(fl); await set("cf:fleet", fl); };
  const saveCategories = async (ca) => { setCategories(ca); await set("cf:categories", ca); };
  ...
  if (!user) return (
    <>
      <style>{CSS}</style>
      <style>{"html,body{background:#040406!important;min-height:100vh}"}</style>
  ...
  return (
    ...
    <div className="app">
      ...
      <div className="hdr">
        ...
        <div className="hdr-r">
          <div className="uchip">
            ...
            {isAdmin && <span className="abadge">Admin</span>}
          </div>
      ...
      {!selectedEvent && (
        <div className="nav">
          {isAdmin ? ATABS.map(t => (
            <button key={t} className={"ntab" + (adminTab === t ? " on" : "")} onClick={() => setAdminTab(t)}>{t}</button>
          )) : (
            <button className="ntab on">My Events</button>
          )}
        </div>
      )}
      ...
      <div className="main">
        {selectedEvent ? (
          ...
          <ActivityLog events={events} users={users} />
        ) : adminTab === "Team" ? (
          <UserManager users={users} onUpdate={saveUsers} />
        ) : adminTab === "Gear Library" ? (
          <MasterItemList masterItems={masterItems} onUpdate={saveMaster} />
        ) : adminTab === "Fleet" ? (
          <FleetLibrary fleet={fleet} onUpdate={saveFleet} />
        ) : adminTab === "Categories" ? (
          <CategoryManager categories={categories} onUpdate={saveCategories} />
        ) : null}
      </div>
